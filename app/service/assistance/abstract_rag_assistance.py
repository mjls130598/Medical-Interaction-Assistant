...
class AbstractRAGAssistance(ABC):
    ...
    def _format_ai_response(self, ai_response: str, source_metadata: list,
                            timestamp: datetime) -> dict:
        """
        Format the AI response to include source citations and a bibliography 
        section based on the retrieved context.
        
        Arguments:
            ai_response (str): The raw response generated by the AI model.
            source_metadata (list): A list of metadata dictionaries for the 
                                    sources retrieved as context.
        
        Returns:
            str: The formatted AI response with citations and bibliography.
        """
        found_indices = sorted(list(set(map(int, re.findall(r'\[(\d+)\]', ai_response)))))
        used_sources = []
        
        for idx in found_indices:
            list_idx = idx - 1 
            
            if 0 <= list_idx < len(source_metadata):
                used_sources.append({
                    "index": idx,
                    "url": source_metadata[list_idx]["source"],
                    "med_name": source_metadata[list_idx]["med_name"],
                    "section_title": source_metadata[list_idx]["section_title"],
                    "verified": True
                })
